...
├── .github/workflows/
│   ├── ios-tests.yml                # INCUMBENT; macos-26 xcodebuild test, path-filtered to Apple inputs; workflow_call for nightly
...
| Three-night outlook day composition | `observing_night.compose_outlook`, `observing_night.select_best` | Unreleased 1.0 | Implemented in the outlook slice: the three observing dates, their boundaries, per-night semantic availability and deterministic best-night selection. The Bot host composes those facts as `agent.outlook`. Widget labels, verdicts, tone, AppGroup persistence, widget cache and timeline scheduling are iOS-only and are not portable Astronomer work. |
...
Argparse/typer, env, `--atlas-path` (defaults to the bundled production LPATLAS1 on the bot VM), stdin/stdout framing, `--pretty`, logging, HTTP client, composed `agent.conditions`, `agent.recommendations`, `agent.outlook`, and `agent.batch_compare` (implemented in source in `packages/astro-host-python`), optional `agent.forecast_horizon` (an out-of-contract arbitrary-N Bot product extension; not yet built), `--suitability-json` overlay file.
...
flowchart TB
  ...
  Change --> Apple["apps/ios/** or packages/astro-engine-swift/** or contracts/data/** or scripts/bundle-engine-data"]
  ...
  Nightly[cron / workflow_dispatch] --> IOSFull
...
**Incumbent iOS paths (Phase 10 decision, superseded 2026-09-11):** Phase 10 deliberately did **not** path-filter `ios-tests.yml` because the pre-Phase 13 root layout had no stable Apple-input set; Python-only commits ran iOS XCTest (over-test, accepted) and the decision was to revisit after Phase 13. That revisit happened once `apps/ios` existed: the trigger set above is derived from `apps/ios/project.yml`, `scripts/bundle-engine-data`, and the iOS tests' own file reads, not from directory names. **Current policy:** `ios-tests.yml` push/PR runs only for Apple product/test inputs; Swift-engine/package and bundled canonical-data changes still run iOS XCTest; Python-host-only and docs-only changes do not; `ios-nightly.yml` is the unconditional scheduled/manual full-XCTest safety net.
...
The future rows above are illustrative engine capabilities only: `agent.*` operations remain host operations and do not enter `contracts/capabilities.yaml`. `since` is assigned when an engine capability is introduced. If it is introduced before the first public release, it uses the unreleased `1.0.0` identity; existing historical `since` values are not rewritten.
...
#### Implemented host slices and H14 acceptance candidate (2026-09-08 through 2026-09-11)

Host slices through H13 have landed: the `agent.conditions` composition (2026-09-08),
the durable weather cache (2026-09-09), saved-location persistence (2026-09-09), place resolution plus selected-location composition (2026-09-10), saved-equipment persistence plus `get_active` (H12, 2026-09-10), and target-recommendation host composition (H13, 2026-09-10). Roadmap item 9
(`agent.outlook`, 2026-09-13) composes the canonical three-night outlook over that
same acquisition path. `packages/astro-host-python` now exists
with composed `agent.conditions` / `agent.locations` / `agent.places` / `agent.equipment` / `agent.recommendations` / `agent.outlook`, and `apps/cli/astro-host`
...
- **Optional later host product extension (proposed, not catalogued):** an arbitrary-N forecast surface — provisionally named `agent.forecast_horizon` — may request more Open-Meteo days than the production three-night product if the Bot needs that capability. It is not required for production compatibility: `agent.outlook` is the canonical production-shaped three-night answer. The extension remains out of contract, has no `contracts/capabilities.yaml` row, and would live in `packages/astro-host-python`.
...
4. **Forecast horizon — Resolved: an arbitrary-N Python host surface is an optional agent-only product extension, not a production-compatibility requirement.** Provisionally named `agent.forecast_horizon`, it remains out of contract and never belongs in `contracts/capabilities.yaml`. `agent.outlook` already supplies the production three-night product. If the extension is chosen, it lives in `packages/astro-host-python`, reuses the established forecast preparation and per-night scoring path, and does not require iOS parity.
...
- **Notes:** No committed-copy `--check`. Package/resource changes still hit incumbent iOS CI because that workflow stays unfiltered. *(Superseded 2026-09-11: `ios-tests.yml` is now path-filtered; see [CI and semantic-drift detection](#6-ci-and-semantic-drift-detection) and the audit note below.)*
- **Implementation notes (2026-09-05):**
  ...
  - `ios-tests.yml` keeps `macos-26` and every existing step. **No path filter** at this phase. Adding one would skip full iOS XCTest on a missed Apple-relevant path; that weakens the [iOS migration invariant](#ios-migration-invariant) more than it saves minutes. `workflow_call` lets nightly reuse the simulator-selection job instead of copying it. *(Superseded by the 2026-09-11 CI-trigger audit below.)*
...
- **CI-trigger audit (2026-09-11, after Phase 13):**
  - A docs-only commit ran full iOS XCTest, which prompted an audit of every workflow's trigger set against what each job actually builds/tests (project spec, package manifests, test file reads, imports).
  - `ios-tests.yml`: push/PR now path-filtered to `apps/ios/**`, `packages/astro-engine-swift/**`, `contracts/data/**`, `contracts/ENGINE_VERSION`, `scripts/bundle-engine-data`, `THIRD_PARTY_NOTICES.md`, and the workflow file. `workflow_call` is unchanged, so `ios-nightly.yml` still runs the full job unconditionally. The historical "intentionally unfiltered" comment in the workflow was replaced by the dependency rationale.
  - `light-pollution.yml`: `packages/astro-engine-python/src/astro_engine/light_pollution.py` widened to `packages/astro-engine-python/src/**` (the harness import executes `astro_engine/__init__.py`, which imports most of the engine); `contracts/fixtures/providers/lpatlas1/**` removed because the harness never reads it (byte identity is asserted by the engine tests); top-level `tools/light-pollution/*.md` excluded.
  - `swift-engine.yml`, `python.yml`, `parity.yml`: trigger sets verified against the tests' actual inputs and left unchanged (`swift-engine.yml` gained a comment explaining `.gitignore` and the Tools fixtures). `contracts/**` stays unpartitioned in the Swift Engine, Python, and Parity workflows: parity tests parse `contracts/procedures/*.md`, so contract markdown is a test input.
...
- **Implementation notes (2026-09-05):**
  ...
  - Apple CI remained unfiltered at this phase, with generation and testing in `apps/ios` *(path-filtered since the 2026-09-11 CI-trigger audit; see Phase 10)*. Root build/open helpers point to the relocated project. `open_in_xcode.sh` already opened the project before this phase; it does not auto-generate it.
...
`packages/astro-host-python` is listed under **Completed (host-side)**, and the
remaining audit items are listed under **Compatibility-audit items and current status**.
...
10. Semantic cloud-timing classification — `night_conditions.classify_cloud_timing`,
   ...
   Subsequent item #10 archaeology found a separate calibrated eligibility gate
   in `generateSummary`; see the cloud-advisory slice below. English wording
...
[implemented host slices](#implemented-host-slices-and-h14-acceptance-candidate-2026-09-08-through-2026-09-11);
...
- **Item 9 — Three-night outlook host composition (`agent.outlook`, 2026-09-13).**
  Reuses H1–H9 acquisition (Open-Meteo, authoritative IANA timezone, bounded
  retry/backoff, 3600-second fresh TTL, configurable stale-on-error, durable
  cache) rather than cloning the single-night path. `ConditionsService._prepare_forecast`
  plans the production four-day horizon (`OUTLOOK_FORECAST_DAYS = 4`, matching
  `SharedConditionsRepository.forecastDays`), including the after-midnight
  `past_days=1` re-acquisition. `observing_night.compose_outlook` owns the three
  observing dates and `available` / `no_astronomical_night` / `unavailable`
  status; the host scores only `available` nights through the same
  `night_conditions.analyze` + `observing_quality.assess` path as
  `agent.conditions`, looking up brightness once so all three nights share one
  score mode; `observing_night.select_best` owns best-night eligibility,
  highest-score comparison, and earliest-wins ties. Headline score is Observing
  Quality. Natural-language labels (`Tonight` / `Tomorrow` / `best night`) stay
  Bot presentation. iOS widget cache DTOs, AppGroup persistence, last-known-good
  retention, maximum age, widget reloads, WidgetKit timeline scheduling, and
  score tone are **not** Astronomer work and were not ported. This is not
  `agent.forecast_horizon`: no arbitrary-N fetch, no 16-day product. Later
  horizon work should reuse `_prepare_forecast` / per-night scoring rather than
...
| # | Compatibility-audit item | Provisional home | Current status / archaeology |
|---|---|---|---|
| 9 | Three-night outlook **host/presentation** composition | **Host presentation over engine facts** — **implemented** | Deterministic core **complete**: `observing_night.compose_outlook` and `observing_night.select_best` own the three observing dates, the per-night `available` / `no_astronomical_night` / `unavailable` classification and the best-night tie rule. **Landed** as `agent.outlook` (2026-09-13): one four-day acquisition, engine composition, shared-mode scoring of available nights, and `select_best`. Bot/skill presentation words slot 0 as the active observing night. iOS widget lifecycle — AppGroup persistence, widget cache DTOs, last-known-good retention, maximum age, widget reloads, WidgetKit timeline scheduling, and score tone — is **not** missing Astronomer work; it is iOS-only and out of scope for the Linux Bot host. |
| 10 | Semantic advisory prose built on cloud timing | **Engine eligibility fact + Bot presentation — implemented for `agent.conditions`** | `night_conditions.classify_cloud_timing` remains the classification authority. Further production archaeology found that `NightQualityAnalyzer.generateSummary` first gives whole-night average cloud at the calibrated heavy floor priority, then permits timing advice only for non-poor rating and non-`none` classification. `night_conditions.select_cloud_advisory` now owns that deterministic gate with Swift/Python equality; Swift summary delegates without changing its copy or branch behavior. `agent.conditions` returns the nullable advisory code and the Astronomer skill explains it in natural language. English text is not parity-governed. `agent.recommendations` and `agent.outlook` are unchanged. |
| 11 | Best Nearby / location-set composition | **iOS grid search retained; Bot named-place comparison implemented in source** | iOS retains its existing grid-based Best Nearby search unchanged. The Astronomer Bot instead discovers at most eight credible, named real-world stargazing destinations with available web/browser tools, resolves coordinates, and passes them to generic Host `agent.batch_compare`. The Host does not discover places; it reuses per-location weather/cache/retry/stale and scoring paths, resolves the center's IANA zone and observing-night date once, acquires candidates with three bounded concurrent calls, and loads the LP atlas once per batch. Engine `location.distance` supplies great-circle miles; `location.compose_scores` owns coherent OQ/Night Conditions mode and center deltas; `location.compare` ranks destinations. No center row is returned as a destination. The Engine does not see place names, URLs, access evidence, or web provenance. For this Bot product, Python grid fan-out, Linux land/water suitability, 40-candidate suitability bands, grid-specific weather batching, and `agent.best_nearby` are dropped/deferred. `location.grid` and `location.filter_recommendable` remain documented as valid iOS/Engine architecture. Ordinary clickable Maps URLs are the baseline; native Grok map cards, multi-pin maps, and built-in Places are not assumed without deployed Bot validation. Replacement wheels and a real Bot smoke test remain a release gate. |
| 12 | Optional arbitrary-N forecast product extension | **Host orchestration over existing engine facts, if chosen** | Archaeology **complete**. Production's only cross-night astronomy decision is the exact Three-Night Outlook, now composed for the Bot as `agent.outlook` (item 9). Dashboard future dates and Best Nearby are single-night flows and provide no arbitrary-N production oracle. `agent.forecast_horizon` is therefore not required for production compatibility; build it only if the Bot needs that product capability. If chosen, its host-shaped work is provider-specific horizon planning, a longer-than-four-day Open-Meteo fetch, scoring orchestration beyond the canonical three nights, and presentation. Reuse `_prepare_forecast` and the per-night scoring helpers from item 9; do not clone them and do not treat `agent.outlook` as that later operation. No new engine capability is required. |
| 13 | Provider availability, failure and staleness semantics | **Host-shaped; current Bot flows implemented** | Classification unchanged: fetch, retry, cache lifecycle, error surfaces and freshness/lifecycle policy are host-shaped by the [placement criterion](#placement-criterion-portable-semantics-vs-operational-policy). Swift has its own policies; the Python host carries no parity obligation to match them. `agent.conditions`, `agent.outlook`, and `agent.batch_compare` reuse Open-Meteo retry/backoff, the 3600-second fresh TTL, configurable stale-on-error with visible `degraded` provenance, distinguishable provider attempt/failure/payload states, and the durable multi-location snapshot cache. Batch compare bounds candidate acquisition at three concurrent requests and preserves partial results. Further provider lifecycle work belongs only to future products such as optional `agent.forecast_horizon`. Astro-domain distinctions remain Engine-owned. |
| 14 | Bot host persistence: saved locations and the selected/default location, place resolution/onboarding/aliases/confirmation, selected equipment, user state, observation history | **Host** | Settled by design. The engine is stateless with respect to user history and does not own saved-location persistence. Grok may personalize over host state but must not fold it into deterministic scoring. **H10 has landed** the saved-location directory, selected/default FK, aliases, mandatory IANA, and `agent.locations` CLI. **H11 has landed** place resolution, confirmation-save of the shown candidate, and selected-location composition into `agent.conditions`. **H12 has landed** the saved-equipment inventory, three-way selection policy, aliases, `get_active` engine-shaped projection, and `agent.equipment` CLI. H14 implements the first onboarding dialogue but remains pending replacement-wheel clean/fresh-user bootstrap and operation acceptance. Still outstanding: user profile, observation history, and broader onboarding. `FileWeatherCache` (H7–H9) remains a provider snapshot cache, not this store. |
...
**Item #10 cloud-advisory correction (2026-09-13).** The earlier cloud-timing
slice correctly moved run classification into `night_conditions.classify_cloud_timing`
but missed a second deterministic rule in Swift presentation: whole-night average
cloud cover at the calibrated heavy floor takes priority, then a non-poor rating
and non-`none` classification are required before timing advice leads the
summary. The parity-governed `night_conditions.select_cloud_advisory` capability
now returns a nullable semantic code over those three existing facts. Swift's
summary delegates to the same selector; `agent.conditions` exposes its result;
the Astronomer skill supplies English wording. The classifier's `none` does not
mean clear skies. Exact Swift copy remains outside parity. No `agent.outlook` or
`agent.recommendations` presentation path was added for this item.
...
Also outstanding and unambiguously host: the rest of
durable user state (profile, observation history); deployed Bot discovery validation
and Bot-facing presentation; and H14's replacement-wheel, fresh-user bootstrap
and operation acceptance cycle. `agent.forecast_horizon` is optional product
extension work, not a compatibility requirement. Composed `agent.conditions`,
`agent.recommendations`, and `agent.outlook` are **no
...
than reclassified. H14 remains open pending replacement-wheel clean/fresh-user
bootstrap and operation acceptance.
...
presentation. **At the time of this correction**, it had no Python implementation,
no capability row, and no field in the `night_conditions.analyze` DTO or equality
policy. The earlier note that it
...
surface it and wording it — plus Best Nearby, optional arbitrary-N multi-night
composition (`agent.forecast_horizon`, only if the Bot needs that extension), provider availability semantics for
those later flows, and Bot readiness. The deterministic core of the three-night
outlook landed in the slice below; its Bot host composition landed as
`agent.outlook` (item 9). iOS widget lifecycle is not outstanding Astronomer work.
...
Those host gaps have since been closed for the Bot three-night product:
timezone acquisition, provider availability/failure/staleness for the
single-location Open-Meteo flow, and `agent.outlook` itself (item 9). iOS widget
cache DTOs, AppGroup persistence, last-known-good retention, maximum age,
widget reloads, WidgetKit timeline scheduling, and score tone remain iOS-only
and are **not** missing Astronomer work. Best Nearby, and `agent.forecast_horizon`
if the optional extension is built, must **reuse** these two engine capabilities
and the outlook acquisition helpers rather than re-derive them. The current
reading is in the boundary below
and in
[Completed (host-side)](#required-implementation-order-after-the-audit); the
...
**Current host-composition state.** `packages/astro-host-python` now has three
landed production-shaped single-location/outlook answers delivered through
`apps/cli/astro-host`: `agent.conditions` composes authoritative IANA timezone
acquisition, Open-Meteo acquisition, retry/cache policy, the exact night window,
Night Conditions, best window and cloud-timing fact; `agent.recommendations`
composes Moon, Venus, Mars, Jupiter, Saturn and all 29 deep-sky objects through
the engine recommendation authorities and equipment filtering; and
`agent.outlook` reuses the same acquisition path for the canonical three-night
product. Source-tree `agent.batch_compare` composes caller-supplied named places
for the center-defined night. See [Completed (host-side)](#required-implementation-order-after-the-audit).

The item #10 cloud-advisory eligibility and Astronomer presentation have since
landed for `agent.conditions`. Bot item #11 source implementation has landed;
iOS grid-based Best Nearby remains unchanged. Still missing are user profile
and observation history (selected equipment persistence has landed as H12);
broader Bot-facing presentation and real deployed discovery validation;
replacement wheels from the then-current source tree, shared-skill transport
verification, clean/fresh-user bootstrap and operation acceptance testing; or
precipitation in the normalized weather domain. Optional future work includes
`agent.forecast_horizon` only if the Bot needs an arbitrary-N product; it is not
the canonical three-night `agent.outlook`. iOS widget cache, AppGroup persistence, freshness
retention, widget reloads and timeline scheduling are not outstanding Bot work.
Full host orchestration therefore remains incomplete.
...
- CLI/Bot-host composition — all of it lives in `packages/astro-host-python`, is delivered through `apps/cli`, and is never added to the engine CLI's public 1.0 allow-list (see [Python host boundary](#11-python-host-boundary-packagesastro-host-python)). `agent.conditions`, `agent.recommendations`, `agent.outlook`, and source-tree `agent.batch_compare` **have landed**. The latter compares caller-supplied named places in one center-defined night with bounded cache-aware acquisition. Only if the Bot product needs arbitrary-N forecasting would `agent.forecast_horizon` be added.
- host composition of any remaining product answers over facts the engine already owns (`agent.recommendations`, `agent.outlook`, cloud-advisory eligibility, and Bot named-place `agent.batch_compare` have landed in source);
- broader Bot onboarding beyond the first Astronomer skill's H11 place-resolution / confirmation / selected-location and H12 equipment flows; one-off overrides already do not mutate the store;
...
- replacement wheels built from the then-current source tree and real end-to-end Astronomer Bot bootstrap/operation acceptance from a clean/fresh-user state;
...
- **Live target facts and target-specific policies:** the authoritative
  ...
  imposed.
- **Moon: engine and H13 host composition complete.** The lunar slice
  ...
  preserved. H13's `agent.recommendations` supplies the night/provider facts and
  invokes this authority; it does not re-derive the decision. Remaining gate
  concerns are presentation/personalization and release validation.
- **Planets: engine and H13 host composition complete.** Excluding
  ...
  and reasons. H13's `agent.recommendations` supplies the night interval and
  weather facts and consumes the authority without recreating it. Remaining gate
  concerns are presentation/personalization and release validation.
- **Mixed-target composition:** the ordering decision is shared as
  ...
  H13 now globally composes deep-sky, Moon and planet candidates through
  `targets.compose_recommendations`, then applies the landed equipment-filtering
  path before its dashboard truncation. The Bot therefore does not independently
  recreate objective composition, ranking, or filtering decisions. Remaining
  gate concerns are semantic presentation/personalization and release validation.
- **Observing night and time: engine observing-night/window logic complete;
  single-location host composition landed.**
  ...
  to forecast rows. The canonical three-night outlook reuses that same
  acquisition as `agent.outlook`; Bot named-place `agent.batch_compare` reuses
  it for every candidate inside the center's calendar context. An optional
  future `agent.forecast_horizon` product extension would reuse it too if built;
  it is not required for production compatibility.
...
- **Best Nearby and location composition:** iOS retains grid search and its
  suitability eligibility. Bot `agent.batch_compare` accepts named coordinates
  and preserves candidate nighttime-forecast eligibility, center/candidate
  composition, cloud/wind/fog facts, coherent Observing Quality or Night
  Conditions fallback, center improvement, and missing-center behavior. It does
  not apply land/water suitability or treat LP NoData as a land/water classifier.
  iOS grid Best Nearby retains production center-shared Sun/Moon facts; Bot
  named-place comparison evaluates Sun/Moon at each candidate coordinate while
  keeping the center-defined observing calendar and night.
