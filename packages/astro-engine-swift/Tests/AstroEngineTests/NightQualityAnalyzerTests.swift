...
final class NightQualityAnalyzerTests: XCTestCase {
    ...
    func testDeterministicEmptyNightUsesZeroMoonIlluminationAndInjectedWindow() throws {
        let start = NightConditionsEnvelope.parseUTCInstant("2026-03-01T20:00:00Z")!
        let end = NightConditionsEnvelope.parseUTCInstant("2026-03-02T05:00:00Z")!
        let result = try NightQualityAnalyzer.analyzeNight(
            forecasts: [],
        ...
        XCTAssertTrue(result.hourlyRatings.isEmpty)
        XCTAssertEqual(result.rating, .poor)
        XCTAssertEqual(result.details.moonIlluminationAvg, 0)
        XCTAssertEqual(result.nightStart, start)
        XCTAssertEqual(result.nightEnd, end)
        XCTAssertEqual(NightConditionsScoring.publicScore(result), 20)
    }
