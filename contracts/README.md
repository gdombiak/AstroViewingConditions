...
`catalog.deep_sky`, `targets.recommend`, and `equipment.match`. Live astronomy
and composed agent hosts are still later work. Phase 15 procedures describe
...
The public Python CLI allow-lists exactly those fourteen IDs. Capability
...
- Fixture `meta.yaml` `engine_semver` is an **applicability range** (`>=0.1.0 <2.0.0` for original fixtures; new Phase 14/15 fixtures use `>=1.0.0 <2.0.0`). Promoting the engine to 1.0.0 without changing scoring semantics did **not** rewrite goldens.
